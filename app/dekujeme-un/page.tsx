...
function ThankYouContent() {
  ...
  const landingSlug = searchParams.get('landing') || 'wallheater-un-cz';
  const customValue = searchParams.get('value');
  const transactionId = searchParams.get('tid');
  const isDouble = searchParams.get('double') === '1';

  // Konfigurace landing pro Google Ads
  const landingConfig = getLandingConfig(landingSlug);

  // Dynamická hodnota konverze
  const conversionValue = customValue ? parseFloat(customValue) : (landingConfig?.defaultValue || 2490);
  const currency = landingConfig?.currency || 'CZK';
  const productName = landingConfig?.productName || 'Nástěnná klimatizace Pro 2v1';
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 mb-6">
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-gray-900">Co se stane dál?</h3>
            ...
            <div className="flex items-start gap-4 p-4 bg-blue-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Náš konzultant vám zavolá do 24 hodin, aby potvrdil objednávku a domluvil detaily doručení.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 p-4 bg-orange-50 rounded-lg">
              ...
              <div>
                <h4 className="font-semibold text-gray-900">2. Odeslání za 24-48 hodin</h4>
                <p className="text-sm text-gray-600">
                  Po potvrzení objednávky bude balíček odeslán kurýrem. Obdržíte číslo pro sledování zásilky.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 p-4 bg-green-50 rounded-lg">
              ...
              <div>
                <h4 className="font-semibold text-gray-900">3. Platba na dobírku</h4>
                <p className="text-sm text-gray-600">
                  Zaplatíte kurýrovi hotově nebo kartou při převzetí balíčku. Bez rizika!
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 sm:p-6 mb-6">
          ...
          <ul className="text-sm text-yellow-800 space-y-1">
            ...
            <li>• Zkontrolujte balíček před zaplacením kurýrovi</li>
